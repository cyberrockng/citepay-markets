...
**Status:** historical plan, with shipped-status annotations updated 2026-07-19. Written 2026-07-08 against the live repo state (commit `4e73070`). Revised same day to add a product-and-distribution layer above the original engineering plan (§5), following review.
...
- **Citation Clearance API** — the core, callable primitive: given a claim, a candidate quote, and a source, return authorized/supported/licensed/paid/refused, with a receipt. *(exists: `POST /api/clear/check`, `GET /api/clear/[id]`, and `POST /api/clear/settle`, with `POST /api/clear/demo-run` retained as the scripted demo path.)*
- **Publisher/Creator Licensing Console** — let a creator register content, set price and license terms, and see their clearance history without needing a broad dashboard. *(ships in a narrow form: `/register` captures license class, price, payout wallet, and domain verification status; `/creator/[wallet]/clearances` lists the creator's clearance history. Broader analytics remain deferred.)*
...
- **Integration SDK/MCP Endpoint** — let any Claude/agent framework call CitePay as a tool. *(exists: `citepay-mcp` on npm, `src/app/api/mcp/route.ts`, original tools `cite_query`/`get_receipt`/`check_policy`/`probe_source`, and Clear tools `clear_claim`/`get_clearance`/`settle_clearance`.)*
- **Clear Badge** — a visible, embeddable trust mark for a cleared or refused citation. *(exists: `src/app/api/clear/[id]/badge/route.ts`, with embed snippets shown on `/clearance/[id]`.)*
...
- **Publisher Policy File ingestion** — machine-readable terms a creator hosts on their own domain. *(exists: `src/lib/clear/wellknown.ts`, consumed by `/api/sources/register-public` at registration time and surfaced as creator verification status.)*
...
2. **CitePay Clear Badge** — a public label shown beside AI citations, proving whether the citation is cleared, cleared and paid, not cleared, or not found.
...
**What it must show, and the discipline that keeps it honest:** the shipped badge renders four public states over the real clearance and settlement records — never a second, parallel vocabulary that could drift from what the code actually decided. Detailed refusal reasons stay on the linked receipt page.

| Badge label (display) | Backing field |
|---|---|
| Cleared Paid | `decision === "CLEARED"`, `amountPaidMicro > 0`, linked receipt is `confirmed`, and the receipt has a tx hash |
| Cleared | `decision === "CLEARED"` without confirmed linked payment evidence |
| Not cleared | clearance exists and `decision !== "CLEARED"` |
| Not found | no clearance exists for the requested id |
...
**Clear-specific library (`src/lib/clear/`):** `evaluate.ts` (`evaluateClaimClearance` — the real pre-payment gate: mandate active → license → source policy → quote verified → support score → price/budget, in that order), `check.ts`/`settle-api.ts` (general Clear API handlers), `quote-verify.ts` (`verifyQuoteSpan` — deterministic, NFKC-normalized substring match; the single load-bearing guarantee in the product), `settle.ts` (`createPaidReceipt` — shared real-settlement path), `recover.ts` (`matchAndEvaluateCandidate`, `auditMandate`), `mcp-tools.ts`, `creator-clearances.ts`, `wellknown.ts`, `hash.ts`, `source-text.ts`, `types.ts`.

**Clear routes:** pages `/clear/demo`, `/clearance/[id]`, `/recover`, `/creator/[wallet]/clearances`; APIs `POST /api/clear/check`, `GET /api/clear/[id]`, `GET /api/clear/[id]/badge`, `POST /api/clear/mandate`, `POST /api/clear/settle`, `POST /api/clear/demo-run`, `POST /api/clear/recover/audit`, `POST /api/clear/recover/settle`, and `GET /api/creator/[wallet]/clearances`.
...
**Security posture as of this update:** Clear mutating routes (`check`, `settle`, `mandate`) require issued `cpk_...` API keys and use shared per-key rate limiters. Public clearance GET surfaces (`/api/clear/[id]` and `/api/clear/[id]/badge`) use shared per-IP rate limiters. `POST /api/clear/recover/audit` is rate-limited and input-capped. `POST /api/clear/recover/settle` requires an existing budget-bearing mandate (never the unlimited audit-only mandate), re-evaluates against that mandate's real rules rather than trusting the audit's optimistic result, checks remaining budget via `getSpentMicroByMandateConfigId`, is replay-guarded via `src/lib/replay-guard.ts`, has a **permanent** duplicate-settlement check (`hasSettledClaim`/`getNeonHasSettledClaim` in `db.ts`/`neon.ts` — added specifically because the time-bound replay guard alone wasn't sufficient for this case), and requires an explicit `confirm: true`. The well-known policy fetch path uses the shared `ssrfSafeFetch` primitive.

**Tests:** `npm test` runs unit, payment, rate-limit, and Clear suites across the current `tests/*.test.ts` coverage. `tsc --noEmit` and `npm run lint` are the local verification commands used alongside the test suite.
...
**Features:** the existing `/register` flow now includes license-class selection, price, payout wallet, and `/.well-known/citepay.json` verification status. A "my clearances" view scoped to one creator wallet now exists at `/creator/[wallet]/clearances`, reusing the existing clearance/receipt aggregation path rather than introducing a broad analytics dashboard.
**Files:** `src/app/register/page.tsx`, `src/app/creator/[wallet]/clearances/page.tsx`, `src/app/api/creator/[wallet]/clearances/route.ts`, `src/lib/clear/creator-clearances.ts`, `src/lib/clear/wellknown.ts`, and existing `src/lib/db.ts` source functions.
**Tests:** `tests/clear-wellknown.test.ts` and `tests/clear-creator-clearances.test.ts`, plus the clearance evaluator tests that assert a registered source's `licenseClass` is respected by `evaluateClaimClearance`.
...
**Features:** `POST /api/clear/check` now accepts general-purpose claim/quote/source clearance checks and returns the same `ClaimClearance` family of fields the demo proved. `POST /api/clear/settle` is the general-purpose settlement route, distinct from `recover/settle`. The existing `citepay-mcp` package and `src/app/api/mcp/route.ts` now expose Clear tools `clear_claim`, `get_clearance`, and `settle_clearance` alongside `cite_query`/`get_receipt`/`check_policy`/`probe_source`.
**Files:** `src/app/api/clear/check/route.ts`, `src/app/api/clear/settle/route.ts`, `src/app/api/mcp/route.ts`, `src/lib/clear/mcp-tools.ts`, and the `citepay-mcp` npm package.
**Tests:** `tests/clear-evaluate.test.ts`, `tests/clear-mcp.test.ts`, `tests/clear-budget.test.ts`, and `tests/clear-recover.test.ts` cover the route/tool decision matrix, idempotency, budget checks, and settlement boundaries.
**Risks:** this is the first Clear route callable by arbitrary keyed third parties — it needs the same rate-limit/input-cap discipline already applied to `recover/audit`, from day one, not retrofitted after a real integration abuses it.
...
**Files:** extend `src/app/opengraph-image.tsx` for clearance pages specifically, extend the creator page from Phase 1, and keep the shipped `src/app/api/clear/[id]/badge/route.ts` SVG generator as the single badge renderer.
...
**Policy-file resolver (§5.4):** `src/lib/clear/wellknown.ts` fetches `{sourceDomain}/.well-known/citepay.json` at registration time only, parses a small fixed schema, and writes the verified result into the existing `sources` license/price fields via registration — it is a registration-time ingestion path, not a runtime dependency of `evaluateClaimClearance`.
...
**Key security risks to close before broad external traffic:** (1) Phase 4's URL-fetch path and §5.2's Watch fetches need SSRF protection before shipping (allowlist or strict fetch constraints — one shared utility, not separate implementations); (2) the Clear API's issued-key auth and shared per-key rate limiting must stay in force for mutating calls as external integrations scale; (3) `clearance_challenges` (Phase 5) must inherit the exact "objective checks only, no LLM adjudication" discipline already proven everywhere else, or the whole system's core credibility claim weakens.
...
**10-minute partner demo** (the one genuinely new assembly — now depends mostly on Phase 5 and Watch landing): publisher sets license policy, including a `/.well-known/citepay.json` file on their own domain (Phase 1/§5.4) → an external API call clears a citation through the general-purpose Clearance API in proof-only mode first, then paid mode (Phase 2/§5.3) → the resulting receipt, creator payout, and badge are shown → `/recover` audits a real external article and finds an unpaid citation (exists today) → a CitePay Watch list surfaces the same finding automatically on its next scheduled run (Phase 4/§5.2) → a challenge is filed and resolved against a real refused clearance, and the challenge/badge presentation decided in Phase 5 is shown. This demo should not be attempted until Phase 5 and Watch are both real — a partner demo with a simulated step anywhere in it undermines the entire pitch.

---

## 16. Hard Truths

- **What's weak today:** the creator-facing side is still thin even after the first onboarding pass. A creator can register content, see payout/clearance history, and copy badge embeds from a receipt, but there is still no notification loop for new citations without manually checking (Phase 4/Watch). Every competitor with a creator-economics story (per this cycle's own competitive analysis) still pressures this surface.
...
- **What must be production-hardened before broader external traffic:** the Clear API's issued-key auth and shared rate limits must remain enforced on mutating calls; future URL-fetch paths (Phase 4/Watch) need the same shared SSRF-safe primitive already used for content and well-known policy fetches; every new persisted entity needs the full Neon-fallback-plus-stable-ID checklist from §11 applied *before* it ships, not discovered live in production a third time; any challenge-driven change to badge presentation needs an explicit product decision rather than an implicit expansion of today's four badge states.
...
### Original must-build-next status
1. **`POST /api/clear/check` — shipped.** General-purpose, non-scripted clearance now calls the shared evaluator and returns receipt-backed decisions for real external callers.
2. **Clear Badge (§5.1) — shipped.** The SVG route is live and the public receipt page includes a copy-paste embed snippet.
3. **Auth/API-key layer for the new public API — shipped.** Clear check/settle/mandate routes use issued `cpk_...` keys and shared per-key rate limiters.
4. **Creator "my clearances" view (Phase 1) — shipped.** `/creator/[wallet]/clearances` lists a creator's real clearance history.
5. **`/.well-known/citepay.json` policy-file ingestion (§5.4) — shipped.** Registration resolves and stores the creator's declared license fields when the domain policy verifies.
...
7. **MCP tools for the Clear surface (`clear_claim`, `get_clearance`) — shipped.** `src/app/api/mcp/route.ts` and the `citepay-mcp` npm package expose the Clear tools alongside the original market tools.
