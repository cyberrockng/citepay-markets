          ...
          {runResult && (
            <div className="space-y-4 pt-2">
              ...
              {runResult.blocked.length > 0 && (
                <div className="space-y-2">
                  <h3 className="text-xs font-semibold text-[#8b8b9e] uppercase tracking-widest">Not Hired</h3>
                  {runResult.blocked.map(b => {
                    const isCapacityLimit = b.reason?.includes("budget_count_limit") || b.reason?.includes("count_limit");
                    return (
                      <div key={b.receipt.id} className={`rounded-lg border px-4 py-2 flex items-center gap-3 text-sm ${
                        isCapacityLimit
                          ? "border-[#3a3a4e] bg-[#1a1a2a]"
                          : "border-red-800/30 bg-red-900/5"
                      }`}>
                        <span className={isCapacityLimit ? "text-[#8b8b9e] font-semibold" : "text-red-400 font-semibold"}>{b.agent.name}</span>
                        <span className={`text-xs px-2 py-0.5 rounded font-mono ${
                          isCapacityLimit ? "bg-[#2a2a3e] text-[#8b8b9e]" : "bg-red-900/30 text-red-300"
                        }`}>
                          {isCapacityLimit ? "CAPACITY LIMIT" : "POLICY BLOCKED"}
                        </span>
                        <span className="text-xs text-[#8b8b9e]/70">{b.reason}</span>
